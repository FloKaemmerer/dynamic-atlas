...
import { getDivinationCardOverlayColor, getOverlayColor } from '@/composable/overlay-color-utils'
...
export function getDivinationCardOverlayCircle(id: string, atlasNodePoint: { x: number; y: number }, colorValue: number) {
  return new Konva.Circle({
    id: `${id}-circle`,
    x: atlasNodePoint.x,
    y: atlasNodePoint.y,
    fill: getDivinationCardOverlayColor(colorValue),
    radius: 20,
    opacity: 1,
  })
}

export function getDivinationCardOverlayRect(id: string, atlasNodePoint: { x: number; y: number }, colorValue: number) {
  return new Konva.Rect({
    id: `${id}-rect`,
    x: atlasNodePoint.x - 13,
    y: atlasNodePoint.y - 38,
